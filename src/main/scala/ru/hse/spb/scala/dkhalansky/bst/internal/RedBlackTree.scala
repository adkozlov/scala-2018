...
object RedBlackTree {
  ...
  def insert[A](a: A, tree: RedBlackTree[A])(
      implicit ordering: Ordering[A]): RedBlackTree[A] =
    insertInternal(a, tree).makeBlack

  def delete[A](a: A, tree: RedBlackTree[A])(
      implicit ordering: Ordering[A]): RedBlackTree[A] =
    deleteInternal(a, tree).makeBlack
  ...
  private def fuse[A](tree1: RedBlackTree[A],
                      tree2: RedBlackTree[A]): RedBlackTree[A] =
    (tree1, tree2) match {
      case (Leaf, t) => t
      case (t, Leaf) => t
  ...
  private def leftBalance[A](tree: Node[A]): Node[A] = tree match {
    case Node(_, Node(Red, t1, x, t2), y, t3) =>
      Node(Red, Node(Black, t1, x, t2), y, t3)
    case Node(_, t1, y, Node(Black, t2, z, t3)) =>
      rebalance(Node(Black, t1, y, Node(Red, t2, z, t3)))
    case Node(
        _,
        t1,
        y,
        Node(Red, Node(Black, t2, u, t3), z, t4 @ Node(Black, l, value, r))) =>
      Node(Red,
           Node(Black, t1, y, t2),
           u,
           rebalance(Node(Black, t3, z, Node(Red, l, value, r))))
    case _ => tree
  }
  ...
  private def rightBalance[A](tree: Node[A]): Node[A] = tree match {
    ...
    case Node(
        _,
        Node(Red, t1 @ Node(Black, l, value, r), z, Node(Black, t2, u, t3)),
        y,
        t4) =>
      Node(Red,
           rebalance(Node(Black, Node(Red, l, value, r), z, t2)),
           u,
           Node(Black, t3, y, t4))
    case _ => tree
  }
