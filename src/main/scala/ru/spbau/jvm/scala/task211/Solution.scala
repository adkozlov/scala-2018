...
sealed trait Nat {
  ...
  type IsSquare[Sub <: Nat] <: Bool
  type SumOfAllGood[CurSum <: Nat] <: Nat
}
...
class O extends Nat {
  ...
  override type IsSquare[_] = True
  override type IsDivisibleBy[_] = True
  override type Mult[_] = O
  override type SumOfAllGood[CurSum <: Nat] = CurSum
}
...
class Neg extends Nat {
  ...
  override type IsSquare[_] = False
...
class S[T <: Nat] extends Nat {
  ...
  override type IsZero[True <: Up, False <: Up, Up] = False
  override type IsSquare[Cur <: Nat] = Minus[Cur]#IsSquare[Cur#Plus[_2]]
  override type SumOfAllGood[CurSum <: Nat] = T#SumOfAllGood[SumOfDivisors[S[T], O]#IsSquare[_1]#Match[CurSum#Plus[S[T]], CurSum, Nat]]
}
...
object Solution {
  ...
  type Calculate[Start <: Nat] = Start#SumOfAllGood[O]
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(toInt[_9#SumOfDivisors[_9, O]])
    println(toBool[_13#IsSquare[_1]])
    println(toInt[_9#SumOfDivisors[_9, O]#IsSquare[_1]#Match[_4#Plus[_9], _4, Nat]])
    println(toInt[_9#SumOfAllGood[O]])
  }
}
